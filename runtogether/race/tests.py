...
class RaceViewTests(TestCase):
    def setUp(self):
        ...
        self.city2 = City.objects.create(
            name="Versailles", latitude=48.8049, longitude=2.1204
        )  # Approx 17km from Paris
        ...
        self.race1 = Race.objects.create(
            name="Race in Paris", city=self.city1, distance=[10, 21]
        )
        self.race2 = Race.objects.create(
            name="Race in Versailles", city=self.city2, distance=[5, 10]
        )
        self.race3 = Race.objects.create(
            name="Race in Marseille", city=self.city3, distance=[42]
        )
    ...
    def test_list_races_unauthenticated_user(self):
        response = a=self.client.get(reverse("race:races"))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.race1.name)
        self.assertContains(response, self.race2.name)
        self.assertContains(response, self.race3.name)
